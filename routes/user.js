...
const baseUrl1 = "http://localhost:3000/api/v1/user_web";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const [response, response2, response3, response7, response6, response10, response12, response14] = 
      await Promise.all([
    ...
    // console.log(response.data, response3.data, response7.data, response6.data, response10.data, response12.data, response14.data);
    ...
    return res.render("home", {
      ...
      data14: response14.data?.isSuccess ? response14.data?.data : [],
      data13: [],
      // data14: [],
      totalCount: ''
    });
...
router.get("/filter", async (req, res, next) => {
  ...
  try {
    ...
    const [response, response2, response7, response6, response10, response14] = 
      await Promise.all([
        axios.request(getData1("/category", "get")),
  ...
  try {
    ...
    if (id1 != 0 && id2 != 0 && modalShown) {
      ...
      const showData = [{ 
        ...
        url: `${baseUrl1}/poll/${id1}/${id2}/${title}` 
      }];
